...
//cleanUp function의 예시
function Hello() {
  useEffect(() => {
    //처음에 컴포넌트가 만들어질 때 호출됨
...
function App() {
  ...
  return (
    <div>
      ...
      />
      {/*!!onChange는 그냥 props 이름.즉 {}안에 들어간 onChange가 (콜백)함수이름 */}
      {/*value는 입력창에 무엇이 보여지게 하는 것인지. 즉 value 덕분에 화면의 입력창과 state 및 입력값이 연결되는 것임 */}
      <h1>{counter}</h1>
