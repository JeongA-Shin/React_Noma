...
function App() {
  return (
    <Router> {/*react-app 측에서 미리 제공하는 컴포넌트임*/}
      <Switch> {/*Swtich는 라우트를 찾는 역할. route는 url에서 https://domain:portnumber/...에서 /...부분을 의미함 */}
        {/*라우트를 찾으면 컴포넌트를 렌더링함, React Router는 다이나믹(동적) url 을 지원해줌-다이나믹: url에 변수를 넣을 수 있다는 의미 */}
        <Route path="/hello"> 
          <h1>Hello</h1>
        </Route>
        <Route path="/movie/:id"> {/*근데 이 route가 path="/" 라우트 밑에 있으면 /movie해도 실행 안 됨. 루트 라우트 위에 있어야 함 */}
